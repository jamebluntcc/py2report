...
if __name__ == '__main__':
    ...
    command.read('sequencing_conf.conf')
    project_num = command.get('parm','project_num')
    report_name = command.get('parm','report_name')
    project_name = command.get('parm','project_name')
    library_name = command.get('parm','library_name')
    reads_length  = command.get('parm','reads_length')
    data_count = command.get('parm','data_count')
    q30_avg = command.get('parm','q30_avg')
    ...
    context = {'project_num': project_num.strip("'"),
                'report_name': report_name.strip("'"),
                'project_name': project_name.strip("'"),
                'library_name': library_name.strip("'"),
                'reads_length': reads_length.strip("'"),
                'data_count': data_count.strip("'"),
                'q30_avg': q30_avg.strip("'"),
                'filter_distrbution_plot': '{./' + find_plot_path(filter_distrbution_dir) + '}',
                'clean_data_barplot': '{./' + find_plot_path(clean_data_barplot_dir) + '}',
                'raw_data_barplot': '{./' + find_plot_path(raw_data_barplot_dir) + '}',
                'mean_quality_distrbution_plot': '{./' + find_plot_path(base_quality_dir) + '}',
                'base_distrbution_plot': '{./' + find_plot_path(base_content_dir) + '}',
                'mean_quality_distrbution_plot_href': '{run:./' + base_quality_dir + '}',
                'base_distrbution_plot_href': '{run:./' + base_content_dir + '}'
               }
    output_tex_file = time.strftime("%Y-%m-%d", time.localtime()) + '_DNAsequencing_report.tex'
    create_template('sequencing_main.txt',output_tex_file,context=context)
